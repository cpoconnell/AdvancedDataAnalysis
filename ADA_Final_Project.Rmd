...
# data for exploration post analysis
# A1C
...
cohort <- nhanes_clean %>%
  drop_na(diabetes) %>%
  select(SEQN, age, sex, race, income, health_status, diff_walk, work_sch, smoker, phys_act, sleep_time, diabetes, act_transport, work_phys_act)
...
table1(~ act_transport + age + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time | diabetes, data = cohort, render.continuous = c(. = 'Mean (SD)'))
```
...
# health status
table(cohort$health_status, cohort$diabetes, useNA = "always")
# difficulty walking
table(cohort$diff_walk, cohort$diabetes, useNA = "always")
# work schedule
table(cohort$work_sch, cohort$diabetes, useNA = "always")
# smoking status
table(cohort$smoker, cohort$diabetes, useNA = "always")
...
# sleep time
table(cohort$sleep_time, cohort$diabetes, useNA = "always")
```
...
table1(~ act_transport + age_cat2 + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time | diabetes, data = cohort)

# first half for PowerPoint slide 1
table1(~ act_transport + age_cat2 + sex + race + income | diabetes, data = cohort)

# second half for PowerPoint slide 2
table1(~ health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time | diabetes, data = cohort)
...
model2<- glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time, data=cohort_complete, family="binomial")
...
model_em <- glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time + sex*act_transport, data=cohort_complete, family="binomial")
...
The likelihood ratio test is not significant (p=0.1646). Therefore, there is no effect modification by sex. 
...
The mean fasting glucose for those with diabetes is higher among those that report active transportation (174.5) than those that do not (158.5). The median fasting glucose is higher as well (154 versus 144).
For those that do not have diabetes, the mean fasting glucose among those that report active transportation is lower (103.4) than those that do not (105.3). The median is slightly lower as well (100.5 versus 102).
...
The mean A1C for those with diabetes is slightly higher among those that report active transportation (7.7) than those that do not (7.3). The median fasting glucose is higher as well (7.2 versus 6.9).
For those that do not have diabetes, the mean A1C among those that report active transportation is just slightly lower (5.5) than those that do not (5.6). The median is slightly lower as well (5.4 versus 5.5).
...
```{r}
model_imp <- with(imp2, glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time, family="binomial"))
...
```{r}
# model with interaction term between sex and active transport
model_imp_em <- with(imp2, glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time + sex*act_transport, family="binomial"))

# comparing models with likelihood-ratio test statistic
D3(model_imp_em,model_imp)
```

The p-value of likelihood-ratio test statistic is non-significant (p=.113). Therefore, no significant effect modification by sex. 
