...
nhanes_small <- merged_file %>%
  select(SEQN, RIAGENDR, RIDAGEYR, RIDRETH3, DMDEDUC2, DMDEDUC3, INDHHIN2, DIQ010, PAQ605, PAQ635, PAD645, PAQ650, PAQ655, PAD660, PAQ665,PAQ670, PAD675, PAD680, DLQ050, DLQ140, OCQ670, OCD150, OCD390G, SLD012, SMQ020, SMQ040, WHD010, WHD020, HUQ010, PAQ610, PAQ620, PAQ625)
```
...
nhanes_clean <- nhanes_small %>%
  ...
  mutate(work_phys_act = if_else(PAQ605 == 2 & PAQ620 == 2, 0,
                                ...
                                if_else(PAQ605 == 1 & PAQ620 == 2 & PAQ610 %in% c(6:7), 3,
                                        ...
                                        if_else(PAQ605 == 1 & PAQ620 == 1 & mod_plus_vig_work %in% c(6:7), 3,
                                                if_else(PAQ605 == 1 & PAQ610 %in% c(1:2), 1,
                                                if_else(PAQ605 == 1 & PAQ610 %in% c(3:5), 2,
                                                if_else(PAQ605 == 1 & PAQ610 %in% c(6:7), 3, 
                                                if_else(PAQ620 == 1 & PAQ625 %in% c(1:2), 1,
                                                if_else(PAQ620 == 1 & PAQ625 %in% c(3:5), 2,
                                                if_else(PAQ620 == 1 & PAQ625 %in% c(6:7), 3, NULL))))))))))))))))) %>%
...
table(nhanes_clean$PAQ605, nhanes_clean$phys_act, useNA = "always")
table(nhanes_clean$PAQ610, nhanes_clean$phys_act, useNA = "always")
table(nhanes_clean$PAQ620, nhanes_clean$phys_act, useNA = "always")
table(nhanes_clean$PAQ625, nhanes_clean$phys_act, useNA = "always")
...
cohort <- nhanes_clean %>%
  drop_na(diabetes) %>%
  select(SEQN, age, sex, race, educat, income, health_status, diff_walk, work_sch, smoker, phys_act, bmi, sleep_time, sed_time, diabetes, act_transport, work_phys_act)
...
Per DAG, only including the following variables for adjustment in model: General Health Status, age, cigarette use, difficulty walking, income, race/ethnicity, recreational physical activity, sex, sleep, work physical activity, work schedule
![](ADA_Final_Project_DAG.png) 

## Dropping Missing Data for covariates
```{r}
# dropping NA's for covariates needed for the model 
cohort_complete <- cohort %>%
  drop_na(age, sex, race, income, health_status, diff_walk, work_sch, smoker, phys_act, sleep_time, work_phys_act)
```
...
```{r}
# non-imputed model
model1<- glm(diabetes ~ act_transport + age + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + sleep_time + work_phys_act, data=cohort_complete, family="binomial")
...
model_age <- glm(diabetes ~ act_transport + age + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + sleep_time + work_phys_act + age.times.logage, data=cohort_complete, family="binomial") 
...
label(cohort_complete$work_phys_act)<- "Work Physical Activity Level"
label(cohort_complete$sleep_time)<-"Weekday Sleep Amount"
label(cohort_complete$act_transport)<-"Active Transportation"



table1(~ age_cat + sex + race + income + health_status + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time + act_transport | diabetes, data = cohort_complete, render.continuous = c(. = 'Mean (SD)'))
```

Those 18-29 that have diabetes only has 7 observations. Therefore, age categories need to collapse. Excellent health status in the diabetes "yes" column only has 6 observations. Therefore, health status needs collapsed. 
...
table1(~ age_cat2 + sex + race + income + health_status2 + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time + act_transport | diabetes, data = cohort_complete, render.continuous = c(. = 'Mean (SD)'))
```

## New model with categorical age and health status
```{r}
# model with categories
model2<- glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status2 + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time, data=cohort_complete, family="binomial")
...
model_em <- glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status2 + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time + sex*act_transport, data=cohort_complete, family="binomial")
...
The likelihood ratio test is not significant (p=0.2159). Therefore, there is no effect modification by sex. 
...
cohort_imp <- nhanes_clean %>%
  ...
  select(SEQN, age, sex, race, educat, income, health_status, diff_walk, work_sch, smoker, phys_act, work_phys_act, sleep_time, diabetes, act_transport, income_underover)
...
```{r}
model_imp <- with(imp2, glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status2 + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time, family="binomial"))
...
```{r}
# model with interaction term between sex and active transport
model_imp_em <- with(imp2, glm(diabetes ~ act_transport + age_cat2 + sex + race + income + health_status2 + diff_walk + work_sch + smoker + phys_act + work_phys_act + sleep_time + sex*act_transport, family="binomial"))

# comparing models with likelihood-ratio test statistic
D3(model_imp_em,model_imp)
```

The p-value of likelihood-ratio test statistic is non-significant (p=.142). Therefore, no significant effect modification by sex. 
...
grViz("digraph flowchart {
      ...
      [2]: 'Excluding 3,398 individuals without active transportation data n=5,856'
